...
/* USER CODE BEGIN PFP */
static void APP_voidEraseRestoreHeaderPage(uint32_t Copy_u32Address, uint32_t Copy_u32NewData);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if ((RxData[0] == 0x10))
  {
	  APP_voidEraseRestoreHeaderPage(0x0801FC00, 0x00000000);
	  MX_IWDG_Init();
  }
}
/* USER CODE END PFP */
...
int main(void)
{
  ...
  HAL_GPIO_WritePin(Head_Light_GPIO_Port, Head_Light_Pin, GPIO_PIN_SET);
  ...
  {
    /* USER CODE END WHILE */
	  while(HAL_GPIO_ReadPin(Brake_GPIO_Port, Brake_Pin) == GPIO_PIN_RESET)
	  {
...
static void MX_CAN_Init(void)
{
  ...
  canfilterconfig.FilterIdHigh = 0x060<<5;
  canfilterconfig.FilterIdLow = 0;
  canfilterconfig.FilterMaskIdHigh = 0x060<<5;
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, Head_Light_Pin|Back_Light_Pin|Brake_Light_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : Head_Light_Pin Back_Light_Pin Brake_Light_Pin */
  GPIO_InitStruct.Pin = Head_Light_Pin|Back_Light_Pin|Brake_Light_Pin;
...
static void APP_voidEraseRestoreHeaderPage(uint32_t Copy_u32Address, uint32_t Copy_u32NewData)
{
	uint32_t Local_u32AddressArray	[20];
	uint32_t Local_u32DataArray		[20];
	...
	for( Local_u32AddressCounter = 0x0801FC00 ;Local_u32AddressCounter < 0x0801FC50;)
	{
		if( (Local_u32AddressCounter != Copy_u32Address) & (*((volatile uint32_t*)(Local_u32AddressCounter)) != 0xFFFFFFFF))
		{
		...
		Local_u32AddressCounter = Local_u32AddressCounter + 4;
	}
	...
	Local_eraseInfo.PageAddress = 0x0801FC00;
